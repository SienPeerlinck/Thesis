...
class Felics(object):
    ...
    def __init__(self, delta_min = DEFAULT_DELTA_MIN, *args):  # *args toevoegen als temp fix
    ...
    def decode(cls, byte_arr, *args, **kw):
        return cls(*args, **kw).__decode(byte_arr, *args)

    def __encode(self, im, *args):
        bs = BitStream()
        se_code = SubexponentialCode(bs)
        ab_code = AdjustedBinCode(bs)
        if len(args) >= 1:
            if type(args[1]) == Context:
                se_code = SubexponentialCode(bs, args[1])
    ...
    def __decode(self, byte_arr, *args):
        ...
        im = np.empty((height, width), np.uint32)
